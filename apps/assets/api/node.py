...
class NodeChildrenApi(mixins.ListModelMixin, generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        brother_keys = [child.key for child in instance.get_children()]
        if value in values:
        ...
        sub_key = 1
        while 1:
            child_key = "{0}:{1}".format(parent_key, sub_key)
            if child_key in brother_keys:
                sub_key += 1
                continue
            node = Node.objects.create(value=value, id=_id, key=child_key)
            break
